    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <h1 style={{ fontSize: '1.5rem', fontWeight: 600 }}>Classes</h1>
                <Link
            ...
            {loading ? (
                <div style={{ padding: '2rem', textAlign: 'center', color: '#666' }}>Loading classes...</div>
            ) : error ? (
                ...
                <div style={{ padding: '3rem', textAlign: 'center', background: 'white', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                    <p style={{ color: '#6b7280', marginBottom: '1rem' }}>No classes found.</p>
                </div>
            ) : (
                        ...
                        <div key={cls.id} style={{ background: 'white', padding: '1.5rem', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                            ...
                            <div style={{ fontSize: '0.875rem', color: '#374151', marginBottom: '1rem' }}>
                                ...
                                <div style={{ marginTop: '0.5rem' }}>
                                    ...
                                    {cls.class_teachers?.find(t => t.is_homeroom)?.teachers?.users?.full_name || <span style={{ color: '#9ca3af', fontStyle: 'italic' }}>Unassigned</span>}
                                </div>
                            </div>

                            <div style={{ display: 'flex', gap: '0.5rem' }}>
                                <Link
                                    href={`/admin/classes/${cls.id}`} // We'll assume a class detail page if needed later
                                    style={{
                                        padding: '0.5rem',
                                        background: '#f3f4f6',
                                        color: '#374151',
