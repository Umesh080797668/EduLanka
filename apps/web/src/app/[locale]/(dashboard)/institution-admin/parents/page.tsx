    ...
    return (
        <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
                <h1 style={{ fontSize: '1.5rem', fontWeight: 600 }}>Parents & Guardians</h1>
                <Link
            ...
            {loading ? (
                <div style={{ padding: '2rem', textAlign: 'center', color: '#666' }}>Loading parents...</div>
            ) : error ? (
                ...
                <div style={{ padding: '3rem', textAlign: 'center', background: 'white', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                    <p style={{ color: '#6b7280', marginBottom: '1rem' }}>No parents or guardians have been tracked yet.</p>
                </div>
            ) : (
                <div style={{ background: 'white', borderRadius: '8px', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                        <thead style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                            <tr>
                                <th style={{ padding: '0.75rem 1rem', fontWeight: 500, color: '#374151' }}>Parent / Guardian Name</th>
                                <th style={{ padding: '0.75rem 1rem', fontWeight: 500, color: '#374151' }}>Contact</th>
                                <th style={{ padding: '0.75rem 1rem', fontWeight: 500, color: '#374151' }}>Children Linked</th>
                        ...
                        <tbody>
                            {parents.map((parent) => (
                                <tr key={parent.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                                    ...
                                    <td style={{ padding: '0.75rem 1rem' }}>
                                        <div style={{ color: '#4b5563' }}>{parent.phone_number || parent.email || 'N/A'}</div>
                                    </td>
                                    <td style={{ padding: '0.75rem 1rem' }}>
                                        <span style={{
                                            ...
                                            {parent.parent_children?.length || 0} Children
                                        </span>
                                    </td>
